...
Legenda: **P** = positivo · **N** = negativo · **E2E** = fluxo completo · **Smoke** = gate rápido (`npm run testar:smoke`)

| ID | Tipo | Smoke | Endpoint / foco | Cenário | Arquivo |
|----|------|-------|-----------------|---------|---------|
| AUTH-001 | P | sim | `POST /login` | Login com credenciais válidas (JWT) | `testes/login.spec.ts` |
...
| USER-001 | P | sim | `GET /usuarios` | Listar usuários | `testes/usuarios.crud.spec.ts` |
| USER-002 | P | sim | `POST /usuarios` | Criar administrador | `testes/usuarios.crud.spec.ts` |
...
| AUTH-P01 | P | sim | `POST /produtos` | Admin com JWT cadastra produto | `testes/autenticacao.protegidos.spec.ts` |
